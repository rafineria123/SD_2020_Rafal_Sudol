...
public interface PostRepository extends CrudRepository<Post, Long> {
    ...
    @Query(value = "SELECT d.* FROM post AS d LEFT JOIN ( SELECT postId, COUNT(*) AS disc_count" +
            " FROM postcomment GROUP BY postId ) AS c ON d.postId = c.postId ORDER BY  c.disc_count DESC " ,
            nativeQuery = true)
    List<Post> findAllByOrderByCommentDesc();

    @Query(value = "SELECT d.* FROM post AS d LEFT JOIN ( SELECT postId, COUNT(*) AS disc_count" +
            " FROM postcomment GROUP BY postId ) AS c ON d.postId = c.postId where d.tagId in (select tagId from tag where tag.name=?1) ORDER BY  c.disc_count DESC " ,
            nativeQuery = true)
    List<Post> findAllByTagOrderByCommentDesc(String tag);

    @Query(value = "SELECT d.* FROM post AS d LEFT JOIN ( SELECT postId, COUNT(*) AS disc_count" +
            " FROM postcomment GROUP BY postId ) AS c ON d.postId = c.postId where d.shopId in (select shopId from shop where shop.name=?1) ORDER BY  c.disc_count DESC " ,
            nativeQuery = true)
    ...
    @Query(value = "SELECT d.* FROM post AS d LEFT JOIN ( SELECT postId, COUNT(*)" +
            " AS disc_count FROM postcomment GROUP BY postId ) AS c ON d.postId = c.postId" +
    ...
    @Query(value = "SELECT d.* FROM post AS d LEFT JOIN ( SELECT postId, COUNT(*) AS disc_count" +
            " FROM postcomment GROUP BY postId ) AS c ON d.postId = c.postId where d.userId in (select userId from user where login = ?1) ORDER BY  c.disc_count DESC " ,
            nativeQuery = true)
